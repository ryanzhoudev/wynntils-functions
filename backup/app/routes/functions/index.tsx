    ...
    return [
        {
            ...
            apply: (
                fn: wynntilsfunction,
                _args: wynntilsargument[],
                query: string
            ) =>
                (fn.name + " " + fn.aliases.join(" "))
                    .toLowerCase()
...
function RouteComponent() {
    ...
    const renderCard = (func: any) => {
        const filteredArgs: wynntilsargument[] = data.arguments.filter(
            (arg: any) => arg.functionid === func.id
        );
        const argSuffix =
    ...
    return (
        <Docs data={data} filters={getFilters()} renderAction={renderCard} />
    );
}
