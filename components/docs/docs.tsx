...
type FilterCheckbox = {
    id: string;
    label: string;
    apply: (item: any, query: string) => boolean;
...
export default function Docs({ data, filters, renderAction }: DocsProps) {
    ...
    const applyFilters = (items: any[], active: Record<string, boolean>, filters: FilterCheckbox[], query: string) => {
        return items.filter((item) => filters.filter((f) => active[f.id]).some((f) => f.apply(item, query.trim())));
    };
