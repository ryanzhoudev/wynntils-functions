import { type GuildColorApiResponse, type GuildSeasonMetadata, parseAthenaGuildColors } from "@/lib/guild-colors";
import {
...
export async function GET() {
    ...
    try {
        ...
        const { guilds: parsedGuilds, excludedPlaceholderCount } = parseAthenaGuildColors(athenaResult.value.payload);
        ...
        const statsFetchedAt = Date.now();

        const payload: GuildColorApiResponse = {
            guilds,
            fetchedAt: readFetchedAt(athenaResult.value.response),
            cacheSeconds: COLOR_CACHE_SECONDS,
            ...
            source: {
                ...
                etag: athenaResult.value.response.headers.get("etag"),
        ...
        console.error("Failed to load guild colors", error);
