import { TokenKind } from "../parsing/lexer";
import { FunctionCall, ParsedArgument } from "../parsing/parser";
import { functionsCatalog } from "../metadata/functionsCatalog";
...
export function inferArgumentType(argument: ParsedArgument, callLookup: Map<number, FunctionCall>): string | undefined {
  ...
  if (firstToken.kind === TokenKind.Identifier && argument.tokens[1]?.kind === TokenKind.LeftParenthesis) {
    ...
    if (possibleCall && possibleCall.endOffset === argument.endOffset) {
      const metadata = functionsCatalog.findByName(possibleCall.name);
      if (metadata) {
