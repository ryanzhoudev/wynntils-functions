export type CompileResult = { code: string; errors: { off: number; msg: string }[] };
...
export function compileSupersetToWynntils(sourceText: string): CompileResult {
  ...
  const substitutedCode = replacePlaceholders(
    codeWithoutDeclarations,
    (variableName, placeholderOffset) => {
...
function resolveVariables(
  ...
  const resolve = (variableName: string): string | undefined => {
    ...
    const resolvedText = replacePlaceholders(variableInfo.rawValue, (innerName, placeholderOffset) => {
